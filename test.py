...
if __name__ == "__main__":
    ...
    try:
        ...
        # 1. Load and preprocess image for validation FIRST
        IMAGE_PATH = "t.jpg" 
        IMG_SIZE = 224
        ...
        img_to_show = img_array.astype(np.uint8)
        
        # 2. Validate if the image is a plant/leaf
        with LoadingSpinner("Validating image (checking if it's a plant)"):
            # Load small validator model
            validator = MobileNetV2(weights='imagenet')
            
            # Prepare image for ImageNet model
            val_array = np.expand_dims(img_array, axis=0)
            val_array = preprocess_input(val_array)
            
            # Predict
            val_preds = validator.predict(val_array, verbose=0)
            val_results = decode_predictions(val_preds, top=5)[0]
            
            # Check if any top prediction is a plant/leaf/fruit/vegetable
            plant_keywords = [
            ...
            is_plant = any(any(kw in res[1].lower() for kw in plant_keywords) for res in val_results)
            
            # Free validator memory
            del validator
            tf.keras.backend.clear_session()

        if not is_plant:
            print("\n" + "!" * 40)
            print("ERROR: THIS DOES NOT LOOK LIKE A PLANT!")
            print("The AI will not load to save resources.")
            print("Top identifications found:", [res[1] for res in val_results])
            print("!" * 40)
            
            # Show the rejected image anyway
            plt.imshow(img_to_show)
            plt.title("REJECTED: Not a plant/leaf")
            plt.axis('off')
            plt.show()
            sys.exit(0)

        # 3. Load trained model (ONLY if valid)
        MODEL_PATH = "cassava.h5"
        with LoadingSpinner("Initializing Cassava AI Model"):
        ...
        with LoadingSpinner("AI is analyzing image"):
            # Prepare image for our model (divide by 255.0)
